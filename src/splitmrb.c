...
unsigned short GetCWord(FILE *f) {
  ...
  b = getc(f);
  if (b & 1)
    return (((unsigned short)getc(f) << 8) | (unsigned short)b) >> 1;
...
unsigned long GetCDWord(FILE *f) {
  ...
  w = getw(f);
  if (w & 1)
    return (((unsigned long)getw(f) << 16) | (unsigned long)w) >> 1;
...
long copy(FILE *f, long bytes, FILE *out) {
  ...
  for (length = 0; length < bytes; length += size) {
    size = (int)(bytes - length > sizeof(buffer) ? sizeof(buffer)
                                                 : bytes - length);
    ...
    while (bytes-- > 0L) {
      if (!mask) {
        ...
        if (bits & mask) {
          ...
          while (len-- > 0) {
            n += Emit(lzbuffer[pos++ & 0xFFF] = lzbuffer[back++ & 0xFFF],
                      fTarget);
          }
        } else {
    ...
    if (n > 0) {
      ...
      if (hotspot) {
        ...
        for (i = 0; i < n; i++) {
          ...
          printf(" '%s' %d,%d,%d,%d\n", name, hotspot[i].x, hotspot[i].y,
                 hotspot[i].x + hotspot[i].w, hotspot[i].y + hotspot[i].h);
        }
        free(hotspot);
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    fprintf(stderr, "SPLITMRB - splits MRB into SHG/BMP/WMF and SHG into "
                    "BMP/WMF files - Version 1.5\n"
                    "M.Winterhoff <mawin@gmx.net>, Geschw.-Scholl-Ring 17, "
                    "38444 Wolfsburg, Germany\n"
                    "\n"
                    "usage: SPLITMRB filename[.mrb] ...\n"
                    "       SPLITMRB filename.shg ...\n"
                    "\n"
                    "Output files are created in the current directory using "
                    "resolution-dependent\n"
                    "extensions *.EGA,*.VGA,*.CGA,*.854,*.MAC or *.BMP,*.WMF "
                    "or *.Snn,*.nnn (where\n"
                    "n is a digit from 0 to 9). Discarded hotspot info will be "
                    "written to stdout.\n"
                    "\n");
  } else
    for (i = 1; i < argc; i++) {
      ...
      l = strlen(filename);
      while (l > 0 && filename[l - 1] != '\\' && filename[l - 1] != '/' &&
             filename[l - 1] != ':')
        l--;
      m = l;
          ...
          for (j = 0; j < n; j++) {
            ...
            if (byType == 6 || (byType == 5 && byPacked < 2)) {
              ...
              if (dwHotspotOffset && n > 1) // dwHotspotOffset
              {
                ...
                if (fTarget) {
                  fprintf(stderr,
                          "Extracting Segmented Hotspot Graphic '%s'...\n",
                          filename + m);
                  putw(0x506C, fTarget);
                  putw(1, fTarget);
                  putw(8, fTarget);
                ...
                if (fTarget) {
                  fprintf(stderr,
                          "Extracting %ld x %ld x %u Bitmap '%s' (%ldx%ld "
                          "dpi)...\n",
                          bmih.biWidth, bmih.biHeight,
                          bmih.biPlanes * bmih.biBitCount, filename + m, w, h);
            ...
            } else if (byType == 8) // Windows MetaFile
            {
              ...
              fprintf(
                  stderr,
                  "Picture %d of '%s' unknown type %02x pack=%02x skipped.\n",
