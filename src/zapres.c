...
int main(int argc, char* argv[])
{
    ...
    if (b < 2) {
        printf("ZAPRES - removes resolution information from Windows bitmap files "
               "- Version 1.1\n"
               ...
               "usage:	ZAPRES filename1[.BMP] filename2[.BMP] ...\n"
               "\n"
               "option: Normally ZAPRES will create a .BAK file before it attempts "
               "to modify\n"
               "	the input file. Specify option /b if you don't want to backup "
               "files.\n"
               "\n"
               "This program removes the resolution information from Windows .BMP "
               "bitmaps, .MRB\n"
               "multi resolution bitmaps, and .SHG segmented hotspot graphics. "
               "This way WinHelp\n"
               "will not rescale the image and MRBC can be applied to specify a "
               "new resolution.\n"
               "\n");
    } else
        for (i = 1; i < b; i++) {
            ...
            if (f) {
                ...
                if (n == 0x4D42) {
                    ...
                    if (w == 0 && h == 0) {
                        ...
                        fwrite(&zero, sizeof(long), 1, f);
                        fprintf(stderr,
                            "Resolution %lux%lu (%ldx%ld dpi) removed from %s\n", w, h,
                            (w * 2) / 79, (h * 2) / 79, path);
                    }
                    ...
                    for (k = 0; k < n; k++) {
                        ...
                        case 6: /* DIB */
                            ...
                            if (w / 2 == 0 && h / 2 == 0) {
                                fprintf(stderr,
                                    "Resolution was already removed from bitmap %u of %s\n",
                                    k, path);
                                ...
                                fprintf(stderr,
                                    "Resolution %lux%lu removed from bitmap %u of %s\n",
                                    w / 2, h / 2, k, path);
                                bb = 1;
                            }
                        }
